...
interface SimilarPostSuggestion {
  _id: string;
  title: string;
}
...
export const PostThreadPage: React.FC = () => {
  ...
  const [similarPosts, setSimilarPosts] = useState<SimilarPostSuggestion[]>([]);
          ...
          fetch(`http://localhost:5000/api/posts/search-similar?postId=${data.post._id}`)
            ...
            .then((similarData) => {
              if (Array.isArray(similarData)) {
                const filtered = similarData.filter((p: SimilarPostSuggestion) => p._id !== data.post._id);

                const uniqueMap = new Map<string, SimilarPostSuggestion>();
                filtered.forEach((p: SimilarPostSuggestion) => uniqueMap.set(p._id, p));
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
            {post.tags && post.tags.length > 0 && (
              <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                {post.tags.map((tag: { _id: string; name: string } | string, idx) => {
