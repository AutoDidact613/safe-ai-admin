...
import { Agent, AgentStatsResponse } from "./types/agent.types";
    ...
    <div className="agent-stats-page">
      ...
      <div className="agent-stats-grid">
        <AgentRankList
          ...
          onSelect={(id) => navigate(`/download-agents/${id}`)}
        />
        <AgentRankList
          title="🆕 חדשים ביותר"
          agents={stats.newest}
          ...
          {agents.map((a, i) => (
            <li key={a._id} className="agent-rank-item" onClick={() => onSelect(a._id)}>
              ...
              <div className="agent-rank-icon">
                {a.icon ? (
                  <div dangerouslySetInnerHTML={{ __html: a.icon }} style={{ width: 32, height: 32 }} />
                ) : (
                  <span>🤖</span>
                )}
              </div>
